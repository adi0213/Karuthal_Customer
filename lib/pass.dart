...
class Pass extends StatefulWidget {
  final String email;
  final String userId;
  const Pass({super.key, required this.email, required this.userId});
...
class _PassState extends State<Pass> {
  ...
  Future<void> _setPassword(String password) async {
    ...
    var response = await http.post(
        Uri.parse(
            "http://104.237.9.211:8007/karuthal/api/v1/usermanagement/users/resetpassword"),
        headers: headers,
        body: jsonEncode(body));
    if (response.statusCode == 200) {
      print("I am ${response.body}");
      if (jsonDecode(response.body)['status'] == 200) {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => Customerregistration(
                  email: jsonDecode(response.body)['result']['email'],
                  token: jsonDecode(response.body)['result']['authtoken'],
                  password: password,
                )));
      }
    }
              ...
              Row(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    Text(
                      _isPasswordVisible ? 'Hide' : 'Unhide',
                      style: TextStyle(
              ...
              Row(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    Text(
                      _isConfirmPasswordVisible ? 'Hide' : 'Unhide',
                      style: TextStyle(
                        color: const Color(0xFF838181),
                        fontSize: 16,
                      ),
