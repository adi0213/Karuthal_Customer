...
class _ServiceNeededState extends State<ServiceNeeded> {
  ...
  Future<List> getServices() async {
    ...
    var newservices = jsonDecode(serviceList.body)['result'];
    return newservices;
  ...
  void initState() {
    super.initState();
    serviceAvailable =
        getServices(); // Call checkServices when the widget is created
    _printServices();
  }
  ...
  void _printServices() async {
    try {
      newservices =
          await serviceAvailable; // Await the future to get the resolved value
