...
class _SelectPatientState extends State<SelectPatient> {
  ...
  Future<List> getPatients() async {
    final apiUrl =
        "http://104.237.9.211:8007/karuthal/api/v1/persona/patientsby/${widget.customerId}";
    ...
    var registeredPatients = jsonDecode(patientList.body)['result'];
  ...
  void initState() {
    super.initState();
    patientsAvailable =
        getPatients(); // Call checkServices when the widget is created
    _printPatients();
  }
  ...
  void _printPatients() async {
    try {
      patients =
          await patientsAvailable; // Await the future to get the resolved value
  ...
  final List<Map<String, int>> _selectedValues = [];
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: patients
                        .map((patient) => Column(
                              children: [
                                _buildPartition(patient['firstName'],
  ...
  Widget _buildPartition(
      String? firstName, String? lastName, int patientIdNum) {
    String finalFirstName = firstName ?? "NA";
    String finalLastName = lastName ?? "NA";
